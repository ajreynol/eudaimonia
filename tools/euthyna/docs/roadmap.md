...
**Publishing:** `euthyna site` builds a report index, a latest-snapshot page,
and dated snapshot pages, with copyable links and CSV/SVG downloads. A dedicated
GitHub Pages workflow publishes them. See [publishing.md](publishing.md).
...
- **Independent of checker generation.** The reports share Eudaimonia's site
  and are linked from its README, but use their own publishing workflow and
  do not add dependencies to a generated checker.
