...
spec = importlib.util.spec_from_file_location(
    "build_site", Path(__file__).resolve().parents[1] / "report_site/build-site.py"
)
...
class ReportSiteTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.source = site.ROOT / "measurement/data/snapshots"
    ...
    def test_every_local_link_resolves_including_under_a_project_prefix(self):
        for page in self.out.rglob("*.html"):
            text = page.read_text()
            self.assertIn("Generated by Euthyna report_site/", text)
    ...
    def test_default_cli_outputs_stay_in_scratch_and_preserve_measurements(self):
        with tempfile.TemporaryDirectory(prefix="euthyna output ") as directory:
            root = Path(directory) / "euthyna"
            shutil.copytree(site.ROOT, root, ignore=shutil.ignore_patterns("scratch", "__pycache__"))
            source = root / "measurement/data/snapshots"
            before = {p.relative_to(source): p.read_bytes() for p in source.rglob("*") if p.is_file()}
            for args in (("site",), ("plot", self.snapshots[0]["id"])):
                result = subprocess.run(
                    [str(root / "scripts/euthyna"), *args], cwd=directory,
                    text=True, capture_output=True,
                )
                self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
            self.assertTrue((root / "scratch/site/index.html").is_file())
            self.assertTrue((root / "scratch/plots" / self.snapshots[0]["id"] / "rules.html").is_file())
            after = {p.relative_to(source): p.read_bytes() for p in source.rglob("*") if p.is_file()}
            self.assertEqual(before, after)
            self.assertFalse((root / "site").exists())
            for path in root.rglob("*.html"):
                self.assertTrue(path.is_relative_to(root / "scratch"), path)
