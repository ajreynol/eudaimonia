...
usage() {
  cat <<'USAGE'
usage: new_checker/bump-eoc.sh [--commit <sha>] [--dry-run]

  --commit <sha>  pin this commit instead of the head of main
  --dry-run       report what would change, write nothing

What moves together (paths relative to new_checker/):
  ...
  new-checker.sh                            LOGOS_SMT_DIGEST
...
DIGEST_FILES=(new-checker.sh templates/install/install-sig.sh.in)
...
OLD_DIGEST="$(sed -n 's/^LOGOS_SMT_DIGEST="\([0-9a-f]*\)"$/\1/p' new-checker.sh)"
...
[ -n "${OLD_DIGEST}" ] || {
  echo "error: no LOGOS_SMT_DIGEST found in new-checker.sh." >&2
