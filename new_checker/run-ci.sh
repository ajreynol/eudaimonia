...
usage() {
  cat <<'USAGE'
Usage: new_checker/run-ci.sh [OPTION]...
...
./new-checker.sh --checker Shared --calculus Hello --spec examples/hello \
  --out "${OUT}" >/dev/null
...
for cfg in "${CONFIGS[@]}"; do
  ...
  if ! ./new-checker.sh --checker "${name}" --calculus "${calc}" \
...
start=$(date +%s)
if ./new-checker.sh --checker Big --calculus Cpc --spec examples/cpc \
