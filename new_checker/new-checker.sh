...
usage() {
  cat <<'USAGE'
Usage: new_checker/new-checker.sh [OPTION]...
...
It is written under new_checker/checkers/ by default, which is not kept
...
Defaults come from new_checker/config.sh, and every option below overrides what is set
...
Options:
  ...
  --out DIR             where to write the project (default: new_checker/checkers/)
...
Examples:
  ...
  new_checker/new-checker.sh --calculus Lra --signature ~/sigs/Lra.eo
...
[ -f "${tool_root}/config.sh" ] && . "${tool_root}/config.sh"
...
# Print paths relative to the caller when possible, so the suggested commands
# work both from the repository root and from outside the tool's directory.
rel() {
  case "$1" in
    "${caller_dir}/"*) printf '%s\n' "${1#"${caller_dir}/"}" ;;
    *) printf '%s\n' "$1" ;;
...
OUT_DIR="${OUT_DIR:-${tool_root}/checkers}"
